...
void l_gameComplete(GhostType *ghost, HunterListType *hunters, EvidenceListType *hunterEvList) {
    ...
    char ghostStr[MAX_STR];
    // What the actual ghost was 
    ghostToString(ghost->class, ghostStr);

    if(!allHuntersLeft) {
        char discoveredGhost[MAX_STR];

        GhostClass huntersEvGhost = ghostBasedOnEvidence(hunterEvList);
        // What the hunters thought the ghost was
        ghostToString(huntersEvGhost, discoveredGhost);
        ...
        if(ghost->class == huntersEvGhost) {
            printf("The ghost was correctly identified to be a %s by the hunters and was actually a %s!\n", discoveredGhost, ghostStr);
            fprintf(logFile, "The ghost was correctly identified to be a %s by the hunters and was actually a %s!\n", discoveredGhost, ghostStr);

            printf("%-40s\n\n", "The hunters have won the game.");
            fprintf(logFile, "%-40s\n\n", "The hunters have won the game.");
        } else {
            printf("The ghost was incorrectly identified to be a %s by the hunters and was actually a %s!\n", discoveredGhost, ghostStr);
            fprintf(logFile, "The ghost was incorrectly identified to be a %s by the hunters and was actually a %s!\n", discoveredGhost, ghostStr);

            printf("%-40s\n\n", "The ghost won and will continue to haunt the house.");
            fprintf(logFile, "%-40s\n\n", "The ghost won and will continue to haunt the house.");
        }
    } else {
        printf("The hunters were unable to determine that the ghost was a %s!\n", ghostStr);
        fprintf(logFile, "The hunters were unable to determine that the ghost was a %s!\n", ghostStr);
    ...
    if(huntersGotBored) {
        ...
        printHunters(logFile, boredHunters);
    ...
    if(huntersGotScared) {
        ...
        printHunters(logFile, scaredHunters);
