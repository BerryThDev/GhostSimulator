...
void initGhost(HouseType *house, GhostType **ghost) {
    ...
    switch (ghostClass) {
        case POLTERGEIST:
            addEvidence((*ghost)->evList, EMF, C_TRUE);
            addEvidence((*ghost)->evList, TEMPERATURE, C_TRUE);
            addEvidence((*ghost)->evList, FINGERPRINTS, C_TRUE);
            break;
        case BANSHEE:
            addEvidence((*ghost)->evList, EMF, C_TRUE);
            addEvidence((*ghost)->evList, TEMPERATURE, C_TRUE);
            addEvidence((*ghost)->evList, SOUND, C_TRUE);
            break;
        case BULLIES:
            addEvidence((*ghost)->evList, EMF, C_TRUE);
            addEvidence((*ghost)->evList, FINGERPRINTS, C_TRUE);
...
void dropEvidence(GhostType *ghost) {
    ...
    addEvidence(ghost->currentRoom->evidenceList, randEv, C_TRUE);
    sem_post(&ghost->currentRoom->roomSem);
