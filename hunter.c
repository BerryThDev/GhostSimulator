...
/*  Function: initHunter()
    ...
    Returns: None
*/
void initHunter(HunterType **hunter, HouseType *house, char name[], int *id, EvidenceType ev) {
...
void collectEvidence(HunterType *hunter) {
    ...
    addEvidence(hunter->sharedEv, ev, C_FALSE); 
    sem_post(&hunter->sharedEv->evSem);
...
int review(HunterType *hunter) {
    if (!hunter || !hunter->sharedEv) return C_FALSE; // Check for NULL pointers
    sem_wait(&hunter->sharedEv->evSem);
    int size = hunter->sharedEv->size;
    sem_post(&hunter->sharedEv->evSem);
...
/*  Function: printHunters()
    Description: Print out all of the hunters in the list to the console and to the file

    in/out: HunterListType *hunterList - Pointer to the HunterListType to print
    
    Returns: None
*/
void printHunters(FILE *logFile, HunterListType *hunterList) {
    const char printFormat[] =  " - %-37s\n";
    
    // Loop through the hunters and print them to the console and log file
    for(int i = 0; i < hunterList->size; i++) {
        char *name = hunterList->hunters[i]->name;
        printf(printFormat, name);
        fprintf(logFile, printFormat, name);
    }
}
