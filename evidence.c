...
/*  Function: addEvidence()
    ...
    Returns: None
*/
void addEvidence(EvidenceListType *evidenceList, EvidenceType evidenceType, int allowRepeats) {
    ...
    if(!allowRepeats) {
        ...
        getFirstEvidenceNode(evidenceType, &prevNode, &currEv);
...
/*  Function: getFirstEvidenceNode()
    Description: Finds the first node in the list that matches the specified piece of evidence

    in: EvidenceType evidenceType - The EvidenceType to find in the list
    in/out: EvidenceNodeType prevNode - This will become the previous node to the one we searched for
    in/out: EvidenceNodeType nextNode - This will become the exact node we were looking and 
                                        should be set to the start position by the caller
    
    Returns: None 
*/   
void getFirstEvidenceNode(EvidenceType evidenceType, EvidenceNodeType **prevNode, EvidenceNodeType **currNode) {
    // Loop through the list until the given EvidenceType is found
    while (*currNode && (*currNode)->data != evidenceType) {
        (*prevNode) = (*currNode);
        (*currNode) = (*currNode)->next;
    }
}
...
/*  Function: toBitArray()
    Description: Convert an evidence array into a single byte (only unique elements) 

    in: EvidenceListType *evidenceList - Pointer to the EvidenceListType to read
    
    Returns: The array represented as a byte with the enum values bits set to 1
*/
unsigned char toBitArray(EvidenceListType *evList) {
    EvidenceNodeType *currNode = evList->head;
    unsigned char arrayBits = 0; 
    
    // Loop through the list
    while(currNode) {
        // Set each bit based on the enum value
        arrayBits = setBit(arrayBits, currNode->data); 
        currNode = currNode->next;
    }
...
/*  Function: ghostBasedOnEvidence()
    Description: Uses an evidence array to see which type of ghost it should be

    in: EvidenceListType *evidenceList - Pointer to the EvidenceListType to read
    
    Returns: The type of ghost it should be based on the evidence 
*/
GhostClass ghostBasedOnEvidence(EvidenceListType *evList) {
    // Make sure it is possible to figure out the ghost
    if(!evList || evList->size != 3) return GH_UNKNOWN;
    // Convert the array to bits for easy calculation
    unsigned char arrayBits = toBitArray(evList);
    
    // Figure out which ghost type matches the evidence
    if((arrayBits ^ POLTERGEIST_EV) == 0) {
        return POLTERGEIST;
    }

    if((arrayBits ^ BANSHEE_EV) == 0) {
        return BANSHEE;
    }

    if((arrayBits ^ BULLIES_EV) == 0) {
        return BULLIES;
    }

    if((arrayBits ^ PHANTOM_EV) == 0) {
        return PHANTOM;
    }
