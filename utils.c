...
/*  Function: getBit()
    Description: Gets a specific bit in a byte

    in: byte - the byte that we will get the bit from  
    in: bit - which bit we want to read
...
/*  Function: setBit()
    ...
    in: byte - the byte that will have the bit set in  
    in: bit - which bit we want to set
